...
class TestWorkflowCheckRule:
    ...
    @pytest.mark.parametrize(("jira_issues", "mr_state"), [
        ...
        ([{
            ...
            "key": "VMS-667", "branches": ["master", "vms_5.1_patch"], "state": "In progress",
        }], {
        ...
        ([{
            ...
            "key": "VMS-667",
            "branches": ["master", "vms_5.1_patch", "vms_5.1"],
            "state": "In progress",
        }], {
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}, VMS-667: Multiple Jira Issues",
            "commits_list": [{
        ...
        ([{
            ...
            "key": f"{DEFAULT_JIRA_ISSUE_KEY}1", "branches": ["master"], "state": "In progress",
        }], {
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}: Merge request attached to Jira Issue",
            "commits_list": [{
                "sha": DEFAULT_COMMIT["sha"],
    ...
    def test_jira_issues_are_ok(self, workflow_rule, mr, mr_manager):
        for _ in range(2):  # State must not change after any number of rule executions.
            ...
            assert result in (WorkflowCheckRule.ExecutionResult.rule_execution_successful,
                              WorkflowCheckRule.ExecutionResult.not_applicable)
    ...
    @pytest.mark.parametrize(("jira_issues", "mr_state", "error_count", "errors"), [
        # Merge Request is attached to Jira Issue in wrong state.
        ([{
            "key": DEFAULT_JIRA_ISSUE_KEY, "branches": ["master"], "state": "Open",
        }], {
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}: Merge request attached to Jira Issue",
        }, 1, ["has status 'Open'"]),
        # Merge Request is attached to Jira Issue with assignee different from the MR author.
        ([{
            "key": DEFAULT_JIRA_ISSUE_KEY, "branches": ["master"], "state": "In progress",
            ...
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}: Merge request attached to Jira Issue",
            "author": USERS[0],
        }, 1, ["is different from the assignee"]),
        # Both Jira Issues are in wrong state and the assignee is different from the MR author.
        ([{
            "key": DEFAULT_JIRA_ISSUE_KEY, "branches": ["master"], "state": "Open",
            "assignee": USERS[1]["name"],
        }], {
    ...
    def test_jira_issues_with_warnings(self, workflow_rule, mr, mr_manager, error_count, errors):
        for _ in range(2):  # State must not change after any number of rule executions.
            result = workflow_rule.execute(mr_manager)
            assert result == WorkflowCheckRule.ExecutionResult.heuristic_warnings
    ...
    @pytest.mark.parametrize(("jira_issues", "mr_state"), [
        ...
        ([{
            "key": DEFAULT_JIRA_ISSUE_KEY, "branches": ["vms_4.2"], "state": "In progress",
        }], {
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}: Merge request attached to Jira Issue"
        }),
        ...
        ([{
            ...
            "key": "VMS-667", "branches": ["master"], "state": "In progress",
        }], {
    ...
    @pytest.mark.parametrize(("jira_issues", "mr_state"), [
        ...
        ([
            {"key": DEFAULT_JIRA_ISSUE_KEY, "branches": ["master", "vms_5.1_patch", "vms_5.1"]},
            {"key": "VMS-667", "branches": ["master", "vms_5.1_patch"]},
        ], {
            "title": f"{DEFAULT_JIRA_ISSUE_KEY}, VMS-667: Multiple Jira Issues"
        }),
    ])
    def test_has_bad_or_inconsistent_version_set(self, workflow_rule, mr, mr_manager, jira):
        ...
        for _ in range(2):  # State must not change after any number of rule executions.
            ...
            for comment in mr.mock_comments():
                ...
                condition = (
                    re.search(r"VMS-66[6|7|8]: `fixVersions` is inconsistent", comment)
                    or re.search(r"Bad `fixVersions` .+ VMS-66[7|8]: Version set", comment)
                    or re.search(r"Target branch 'master' does not correspond to any", comment))
