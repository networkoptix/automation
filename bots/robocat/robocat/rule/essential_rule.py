...
class EssentialRule(BaseRule):
    EXECUTION_RESULT = EssentialRuleExecutionResultClass.create(
    ...
    def execute(self, mr_manager: MergeRequestManager) -> EXECUTION_RESULT:
        ...
        if preliminary_check_result == self.EXECUTION_RESULT.merged:
            return preliminary_check_result

        if preliminary_check_result == self.EXECUTION_RESULT.no_commits:
        ...
        if not mr_manager.satisfies_approval_requirements(approval_requirements):
            ...
            return self.EXECUTION_RESULT.not_approved

        if first_pipeline_started or mr_manager.ensure_pipeline_rerun():
            return self.EXECUTION_RESULT.pipeline_started
        ...
        if last_pipeline_status == PipelineStatus.running:
            ...
            return self.EXECUTION_RESULT.pipeline_running
